...
naive_bayes_model.fit(scaled_data_train,y_train)
naive_bayes_model.score(scaled_data_test,y_test)

F1Score = cross_val_score(naive_bayes_model,scaled_data_test,y_test,cv=12,scoring='f1_macro');
print("Logistic Model F1 Accuracy: %0.2f (+/- %0.2f)"%(F1Score.mean(), F1Score.std() *2))

# Test Set Review

y_pred = naive_bayes_model.predict(scaled_data_test)
...
y_pred = naive_bayes_model.predict(scaled_val_data)
